...
public class CouponService {
    ...
    public Coupon createCoupon(String code, Double discountPercentage, Long customerId, Long mealId, LocalDateTime expiryDate) {
        Customer customer = customerRepository.findById(customerId)
                .orElseThrow(() -> new NotFoundException("Nie znaleziono klienta"));

        Meal meal = mealRepository.findById(mealId)
                .orElseThrow(() -> new NotFoundException("Nie znaleziono posiłku"));
    ...
    public void deactivateCoupon(Long couponId) {
        Coupon coupon = couponRepository.findById(couponId)
                .orElseThrow(() -> new NotFoundException("Nie znaleziono kuponu"));
    ...
    public boolean isCouponValid(String code, Long customerId, Long mealId) {
        ...
        if (!coupon.getActive() || coupon.getExpiryDate().isBefore(LocalDateTime.now())) {
            throw new CouponInvalidException("Kupon jest nieaktywny lub wygasł");
        }

        if (!coupon.getMeal().getId().equals(mealId)) {
            throw new CouponInvalidException("Kupon nie jest ważny dla tego posiłku");
        }
